      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="mb-3 flex h-[480px] w-[360px] flex-col overflow-hidden rounded-2xl border border-surface-200 bg-white shadow-card dark:border-surface-800 dark:bg-surface-900"
          >
            {/* Header */}
                ...
                <motion.div
                  ...
                  className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                    m.role === "user"
                      ...
                      : "bg-surface-100 text-gray-700 rounded-bl-md dark:bg-surface-800 dark:text-gray-200"
                  }`}
                >
                  {m.text}
                </motion.div>
            ...
            {/* Input */}
            <form onSubmit={sendMessage} className="flex gap-2 border-t border-surface-200 p-3 dark:border-surface-800">
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask about your spending..."
                className="w-full rounded-xl border border-surface-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:border-surface-800 dark:bg-surface-800 dark:text-white dark:focus:ring-brand-500/20"
              />
