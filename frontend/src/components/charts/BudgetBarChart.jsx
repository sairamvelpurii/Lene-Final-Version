...
function CustomTooltip({ active, payload, label }) {
 ...
 <p className="text-sm font-semibold text-gray-800 ">₹{payload[0].value.toLocaleString("en-IN")}</p>
 </div>
 );
}
...
export default function BudgetBarChart({ data = [] }) {
 ...
 <h3 className="mb-4 text-base font-semibold text-gray-800 ">Budget Recommendations</h3>
 {data.length === 0 ? (
