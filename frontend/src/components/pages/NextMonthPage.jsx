...
const COLOR_MAP = {
  orange:  { bg: "bg-orange-100 dark:bg-orange-500/15", text: "text-orange-600 dark:text-orange-400", ring: "ring-orange-200 dark:ring-orange-500/20", bar: "from-orange-500 to-orange-400" },
  sky:     { bg: "bg-sky-100 dark:bg-sky-500/15",       text: "text-sky-600 dark:text-sky-400",       ring: "ring-sky-200 dark:ring-sky-500/20",       bar: "from-sky-500 to-sky-400" },
  rose:    { bg: "bg-rose-100 dark:bg-rose-500/15",     text: "text-rose-600 dark:text-rose-400",     ring: "ring-rose-200 dark:ring-rose-500/20",     bar: "from-rose-500 to-rose-400" },
  purple:  { bg: "bg-purple-100 dark:bg-purple-500/15", text: "text-purple-600 dark:text-purple-400", ring: "ring-purple-200 dark:ring-purple-500/20", bar: "from-purple-500 to-purple-400" },
  emerald: { bg: "bg-emerald-100 dark:bg-emerald-500/15", text: "text-emerald-600 dark:text-emerald-400", ring: "ring-emerald-200 dark:ring-emerald-500/20", bar: "from-emerald-500 to-emerald-400" },
  pink:    { bg: "bg-pink-100 dark:bg-pink-500/15",     text: "text-pink-600 dark:text-pink-400",     ring: "ring-pink-200 dark:ring-pink-500/20",     bar: "from-pink-500 to-pink-400" },
  gray:    { bg: "bg-gray-100 dark:bg-gray-500/15",     text: "text-gray-600 dark:text-gray-400",     ring: "ring-gray-200 dark:ring-gray-500/20",     bar: "from-gray-500 to-gray-400" },
};
    ...
    <div className="space-y-6">
      ...
      <motion.div {...fadeUp(0)} className="pl-12 lg:pl-0">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Your AI-recommended spending limits for <span className="font-semibold text-gray-700 dark:text-gray-200">{nextMonthName}</span>.
        </p>
      </motion.div>

      {!hasData ? (
        <motion.div {...fadeUp(1)} className="card py-16 text-center">
          <CalendarCheck size={48} className="mx-auto mb-4 text-gray-300 dark:text-gray-600" />
          <p className="text-lg font-semibold text-gray-500 dark:text-gray-400">No spending data yet</p>
        ...
        <>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-gray-800 dark:text-white">{item.cat}</p>
            ...
            <motion.div {...fadeUp(3)} className="card">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {uncovered.map((item) => {
                  const c = COLOR_MAP[item.meta.color] || COLOR_MAP.gray;
                  return (
                    <div key={item.cat} className="flex items-center justify-between rounded-xl bg-surface-100 px-4 py-3 dark:bg-surface-800/40">
                      <div className="flex items-center gap-3">
                        <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${c.bg}`}>
                          <item.meta.icon size={16} className={c.text} />
                        </div>
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-200">{item.cat}</span>
                      </div>
          ...
          {/* Tips section */}
          <motion.div {...fadeUp(4)}
            className="card bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-500/5 dark:to-teal-500/5">
            <h3 className="mb-4 text-base font-semibold text-gray-800 dark:text-white flex items-center gap-2">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                ...
                <div key={tip.title} className="rounded-xl bg-white/60 p-4 dark:bg-white/5">
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{tip.title}</p>
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{tip.desc}</p>
                </div>
              ))}
            </div>
          </motion.div>
